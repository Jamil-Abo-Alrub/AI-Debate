...
4. Configurer :
   - **Repository** : `<votre-utilisateur>/debate-project`
   - **Branch** : `main`
   - **Main file path** : `app.py`
5. Dans **Advanced settings → Secrets**, coller :
   ```toml
   DEEPSEEK_API_KEY = "sk-votre-cle"
   ```
6. Cliquer sur **Deploy**.

L'application est en ligne en quelques minutes. Chaque `git push` sur la branche `main` déclenche un redéploiement automatique.

> **Note sur le stockage.** Streamlit Cloud a un système de fichiers éphémère : les personnages créés depuis l'interface persistent jusqu'au prochain redéploiement. Pour une persistance complète, il faut connecter une base de données externe (Postgres, Supabase…).

---

## Limites connues

- **Citations inventées.** Le LLM peut paraphraser ou inventer des citations qui semblent authentiques. C'est pourquoi le panneau de création expose un formulaire d'édition avant sauvegarde.
- **Figures vivantes.** Pour les personnalités vivantes, l'application affiche systématiquement un avertissement satirique. Les contraintes `hard_constraints` doivent inclure l'interdiction d'inventer des actions ou citations spécifiques attribuables à des individus réels.
- **Filesystem éphémère en production.** Les personnages créés via l'interface ne survivent pas à un redéploiement Streamlit Cloud sans backend de stockage.

---

## Pistes d'évolution

- Phase de **contre-interrogatoire** entre les réfutations
- **Tournoi** : bracket de 4 ou 8 personnages
- **Voix** : synthèse vocale par personnage (ElevenLabs, OpenAI TTS)
- **Persistance** des débats avec lien public partageable
- **Sujet du jour** généré automatiquement à partir de l'actualité
...
Ce projet est fourni à des fins éducatives, satiriques et de recherche. Les propos générés sont fictifs et ne représentent pas les opinions réelles des personnes représentées. L'utilisation à des fins de désinformation, de diffamation ou pour usurper l'identité d'une personne réelle est strictement contraire à l'esprit du projet.
